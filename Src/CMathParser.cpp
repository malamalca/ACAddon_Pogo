		...
		if (!IsWhiteSpace(sSource[iRPos]))
		{
			...
			else if (this->IsValidVariableChar(sSource[iRPos]))
			{
				...
				else
				{
					double dVarValue = 0;
					//Get variable value...
					GS::UniString lsVarName(sVarName, (int)strlen(sVarName), CC_Default);
					if (this->arVariables.ContainsKey(lsVarName)) {
						dVarValue = this->arVariables[sVarName];
					}
					else {
						if (!this->pVariableSetProc || !this->pVariableSetProc(this, sVarName, &dVarValue))
						{
							return this->SetError(ResultInvalidToken, "Variable was not defined: %s.", sVarName);
						}
					}
					//else if (!this->pVariableSetProc(this, sVarName, &dVarValue))
					//{
					//	return this->SetError(ResultInvalidToken, "Variable was not defined: %s.", sVarName);
					//}
...
void CMathParser::AddVariable(GS::UniString variableName, double variableValue)
{
	this->arVariables.Add(variableName, variableValue);
}
